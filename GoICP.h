...
#include "lib_begin.h"
		...
		public:
			...
			void initializeRegistration(const rgbd_pointcloud &inputCloud);
			// initialize and build data structures needed for the distance computation method set by setDistanceComputationMode
			void initializeDistanceComputation(const rgbd_pointcloud &inputCloud);
		...
		protected:
			...
			void buildKDTree();
		private:
			const rgbd_pointcloud *source_cloud;
			const rgbd_pointcloud *target_cloud;
			rgbd_pointcloud temp_source_cloud;

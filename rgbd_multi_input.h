...
namespace rgbd {
	...
	public:
		...
		rgbd_multi_input(const std::string& serial);
		/// attach to the rgbd input device of the given serial
		bool attach(const std::string& serial);
		/// attach to rgbd input device of all the given serials
		bool multi_attach(const std::vector<std::string>& serials);
		...
		bool multi_start(InputStreams, std::vector<std::vector<stream_format>>& multi_stream_formats);
		bool start(const std::vector<stream_format>& stream_formats);

		bool multi_start(const std::vector<std::vector<stream_format>>& multi_stream_formats);
		/// check whether device is started
		bool is_started() const;
		bool is_multi_started() const;
		...
		bool get_frame(InputStreams is, frame_type& frame, int timeOut, int index);
		...
		void map_color_to_depth(const frame_type& depth_frame, const frame_type& color_frame, frame_type& warped_color_frame, int index) const;
		...
		*/
		...
		bool map_depth_to_point(int x, int y, int depth, float* point_ptr, int index) const;
	protected:
		/// store whether camera has been started
		//std::vector<rgbd_driver*> driver_list;
		bool started;
		bool multi_started;
		/// store attached serial
		std::string serial;
		//store attached multiple serials
		std::vector<std::string> serials;
		/// pointer to rgbd input device
		rgbd_device* rgbd;
		std::vector<rgbd_device*> multi_rgbd;
	...
	struct driver_registration
	{
		...
		{
			rgbd_multi_input::register_driver(new T, driver_name);
		}
	};*/
